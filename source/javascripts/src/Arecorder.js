var app = angular.module('Arecorder', ['ngResource', 'ngRoute', 'ui.bootstrap', 'ngAnimate', 'ngFileUpload']);
    ...
    function($scope, $resource, $location, $window, $rootScope, $uibModal, $timeout) {
        ...
        $scope.nextPage = function() {
            ...
            Assignments.query({
                ...
                if ($scope.assignments.length === assignments.length) { //返回的assignments跟目前的一样，也就是没有更多了
                    ...
                    for(i = 0; i < step; i++) {
                        if(assignments[l + i]) {
                            $scope.assignments.push(assignments[l + i]);
                        }
                    }
                }
                $scope.busy = false;
        ...
        $scope.refresh = function() {
            ...
            Assignments.query({
                ...
                if (assignments.length === 0) { //用户第一次登陆，数据库中没有用户的作业记录
                    ...
                    $scope.assignments = [];
                    document.querySelector('.assignmentContainer').classList.add('center');
                } else {
                    ...
                    $scope.assignments = assignments;
                    document.querySelector('.assignmentContainer').classList.remove('center');
        ...
        $scope.update = function(assignment) {   //checkbox改变后更新数据库中的状态
            var Assignments = $resource('/api/assignments/edit/:id', {
                id: '@_id' //这的id后面加@_id代表用PUT方法时url中的:id参数等于下面update方法传入的assignment中的_id属性
            }, {
                update: {
                    method: 'PUT'
                }
